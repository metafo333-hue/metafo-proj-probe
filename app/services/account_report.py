...
def build_report(video: dict[str, Any], account: dict[str, Any],
    ...
    if claims:
        P(f"## {_cn(sec)}、两个提醒:把资质晒出来 + 别踩合规坑")
        P("**① 你简介里的「说法」,建议晒证据(②事实核查)**")
        P(f"你写了带「{'、'.join(claims)}」这类的资质/头衔——是真的就是硬核优势,但**口说无凭**(我没法替你核实真假,看你内容的人也一样)。把**证书、工厂实拍、授牌、媒体报道**晒出来," + ("尤其 B2B 找你合作的人最看这个," if biz else "") + "晒证据比口头说有用得多。")
        P("**② 合规别踩坑(⑥合规)**")
        P("- 背景音乐用抖音曲库的免费商用音乐,别用来路不明的(版权风险·会限流甚至下架)。")
        P("- 资质宣传(年限/非遗/龙头这类)确保有据可查——**虚假宣传是红线**(被举报会掉号)。")
        if biz:
    ...
    # —— 脚本生成入口（分析→生产的核心变现出口）——
    # 调研结论：创作者付费顺序=内容生产外包(第二)>分析工具(第四)
    # 脚本外包市场价200-2000元/条；蝉妈妈AI版¥99/月已验证接受度
    # 这个出口把"读完分析"直接衔接到"帮我做内容"
    sec += 1
    P(f"## {_cn(sec)}、下一步：把分析变成内容")
    P("")
