...
> ⚠️ **stale（2026-06-13 标注）**：本文写于 probe 挂 tencent-sh 时期。**2026-06-11 T5v2 裁定后 probe 部署目标 = probe-a 独立节点**（独立 EIP·第三公网入口·SSO 保护），拓扑详见 [project_unified_entry_topology] 与 `~/.claude/rules/L1-infra/server-roles.md` 机6。下方步骤的目标主机需替换为 probe-a，流程骨架（rsync→venv→env→systemd→nginx）仍可参考。
> 部署在 ~~tencent-sh~~ → **probe-a**（独立公网入口）。远程独立服务，经 L1 http transport 挂母体。
...
部署 + selftest 过线后，把 [registry-snippet.yaml](../../deploy/registry-snippet.yaml) 交母体团队注册（http transport）。母体侧需补 HttpToolClient + identity/verify（见 snippet 注记）。
